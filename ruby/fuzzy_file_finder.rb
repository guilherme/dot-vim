...
class FuzzyFileFinder
  ...
  private
    ...
    def follow_tree(directory)
      raise TooManyEntries if `(find #{directory.name} -type f | wc -l)`.to_i >   ceiling
      files.push(`find #{directory.name} -type f`.split("\n"))
      files.flatten!
      directories = []
      path = directory
      files.map! do |file| 
        ...
        file_name = file.pop
        file_path = file.join("/")
        path = Directory.new(file_path) if path.name != file_path
        FileSystemEntry.new(path, file_name)
      end
      files
    end
